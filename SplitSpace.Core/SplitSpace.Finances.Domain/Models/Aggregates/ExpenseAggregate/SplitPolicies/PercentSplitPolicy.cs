...
public class PercentSplitPolicy : IExpenseSplitPolicy
{
    private readonly ExpenseSplitMethod.Percentages _splitData;

    public PercentSplitPolicy(ExpenseSplitMethod.Percentages splitData)
    {
    ...
    public Result<IReadOnlyList<ExpenseSplit>> CalculateSplits(ExpenseId expenseId, SpaceId spaceId, Money total)
    {
        ...
        var sum = _splitData.Participants.Sum(p => p.Percentage);
        if (sum != 100)
        {
            return Result<IReadOnlyList<ExpenseSplit>>.Failure(
                new Error(ErrorType.FailedPrecondition, "Total split percentage does not equal 100"));
        }
        ...
        {
            var amountToPay = total.Amount * participant.Percentage / 100;
            var split = ExpenseSplit.Create(
                expenseId,
                spaceId,
                participant.UserId,
                new Money(amountToPay),
                new ExpenseSplit.SplitDetail.Percentage(participant.Percentage));
